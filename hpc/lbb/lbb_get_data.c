...
/*   Updated: 2025/05/28 01:26:25 by totommi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*lbb_get_full_stats(t_player player, void *buffer)
{
	...
	if (buffer == NULL)
		return (NULL);
	ft_memset(buffer, 0, MSG_LEN + 1);
	msg = lbb_to_msg(player);
	if (msg == NULL)
		return (buffer);
	ft_strlcpy(buffer, msg, MSG_LEN + 1);
	free(msg);
	return (buffer);
}

/* Expected a buffer of size >(131 * MAXPLAYERS).
...
char	*lbb_get_lobby_stats(t_player *lobby, void *buffer)
{
	...
	if (buffer == NULL)
		return (buffer);
	ft_memset(buffer, 0, 131 * MAXPLAYERS);
	if (lobby == NULL)
		return (buffer);
	...
	while (i < MAXPLAYERS && count < player_count)
	{
		...
		{
			lbb_get_full_stats(lobby[i], &buffer_c[(count * 131) + count]);
			count++;
