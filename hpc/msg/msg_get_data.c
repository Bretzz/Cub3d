...
/*   Updated: 2025/05/28 14:03:38 by topiana-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* expected a buffer of size >MSG_NAME_LEN
...
char	*msg_get_name(const char *msg, char *buffer)
{
	...
	if (buffer == NULL)
		return (NULL);
	ft_memset(buffer, 0, MSG_NAME_LEN * sizeof(char));
	if (msg == NULL)
		return (buffer);
...
/* expected a buffer of size >MSG_IP_LEN
Takes a msg-string and a buffer as parameters.
...
char	*msg_get_ip(const char *msg, char *buffer)
{
	...
	if (buffer == NULL)
		return (NULL);
	ft_memset(buffer, 0, MSG_IP_LEN * sizeof(char));
	if (msg == NULL)
		return (buffer);
