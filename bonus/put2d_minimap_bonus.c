...
/*   Updated: 2025/05/30 17:24:03 by topiana-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	put2d_minimap(t_mlx *mlx, size_t side)
{
    ...
    while (i < MAXPLAYERS)
    {
        if (lbb_is_alive(mlx->lobby[i]) && mlx->lobby[i].extra != NULL)
        {
            put2d_player(mlx, (float *)mlx->lobby[i].pos, side, ((t_sprite *)mlx->lobby[i].extra)[0].chroma);
        }
        i++;
    }
