...
/*   Updated: 2025/05/30 18:29:10 by topiana-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_mouse(t_mlx *mlx)
{
	...
	if (mlx->keys.mouse[0] != MLX_WIN_X / 2)
	{
		mlx->player.dir[0] += (mlx->keys.mouse[0] - (MLX_WIN_X / 2)) * delta_dir;
		moved[0] = 1;
	}
	if (mlx->keys.mouse[1] != MLX_WIN_Y / 2)
	{
		y_diff = (float)(mlx->keys.mouse[1] - (MLX_WIN_Y / 2)) / 6;		// adjust sens
	...
	mlx_mouse_move(mlx->mlx, mlx->win, MLX_WIN_X / 2, MLX_WIN_Y / 2);	// macOS issues
	if (moved[0] || moved[1])
		return (1);
	return (0);
}
