...
static int	put_sprite(t_mlx *mlx, t_plot plot, t_sprite sprite)
{
	...
	win_x_y[1] = 0;
	while (win_x_y[1] < new_heigth && win_x_y[1] + (plot.y_screen - (new_heigth / 2)) < MLX_WIN_Y)
	{
		win_x_y[0] = 0;
		while (win_x_y[0] < new_width && win_x_y[0] + (plot.x_screen - (new_width / 2)) < MLX_WIN_X)
		{
...
int	put_sprite_on_map(t_mlx *mlx, float *pos, t_sprite sprite)
{
	...
	// my_dist = sqrt((pos[0] - my_pos[0]) * (pos[0] - my_pos[0])
	// 	+ (pos[1] - my_pos[1]) * (pos[1] - my_pos[1]));
	// ft_printf("check\n");
	...
	mlx->player.last_sprite_data.width = sprite.width;
	mlx->player.last_sprite_data.heigth = sprite.heigth;
	mlx->player.last_sprite_data.scale = sprite.scale;
